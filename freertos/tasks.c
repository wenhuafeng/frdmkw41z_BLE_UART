...
extern void Uart_AppSendData(uint8_t* Dat, uint8_t Size);   //add by myself
extern uint8_t RxDat[20];        //add by myself
extern uint8_t RxSize;			//add by myself
...
static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
	...
	for( ;; )
	{
		...
		if(RxSize != size)
		{
			...
			{
				size01 = RxSize-size;
				Uart_AppSendData(&RxDat[size], size01);
			}
			else
			{
				size01 = 20-size+RxSize;
				Uart_AppSendData(&RxDat[size], 20-size);
				if(size01>20-size)
					Uart_AppSendData(&RxDat[0], size01-20+size);
			}
			size = size+size01;
			if(size>19)
				size = size-20;
